...
RESET_DATA_SOCKET_CMD = API_URL + "/nic/gpr/data_socket_reset"
...
setup_get_nic_json_response = get_requests(SETUP_CMD, "Setup")
print(setup_get_nic_json_response)
# Confirm we've received a successful response
...
while trace_num < 100:
    ...
    print("Trace Count: {}".format(trace_num))
    ...
    if reset_flag and trace_num >= 50:
        print("Resetting data socket!")
        json_response = put_requests(RESET_DATA_SOCKET_CMD, {}, "Reset data socket")
        # If the command failed, exit the loop waiting for traces
        if json_response['status']['status_code'] != 0:
            print("Error resetting the data socket, stopping collection")
            break
        # Toggle the flag so that the command isn't sent multiple times    
        reset_flag = False

        time.sleep(2)
        # Reconnect to the data socket after waiting
        data_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        data_socket.connect((IP_ADDRESS, data_socket_port))
